...
module Jobs
  ...
  class DisteleplusSendTestMessage < ::Jobs::Base
    def execute(_args)
      ...
      if chat_id.blank?
        DiscourseDisteleplus::Health.record_error("chat not found: disteleplus_telegram_chat_id is blank", context: "test")
        return
      end

      payload = {
        chat_id: chat_id,
        text: I18n.t("disteleplus.test_message", site: CGI.escapeHTML(SiteSetting.title), url: Discourse.base_url + "/disteleplus"),
        parse_mode: "HTML",
      }
      ...
      if result.ok
        ...
        Rails.logger.warn("#{DiscourseDisteleplus::LOG_TAG} test message failed: #{result.description}")
      end
    end
  end
end
